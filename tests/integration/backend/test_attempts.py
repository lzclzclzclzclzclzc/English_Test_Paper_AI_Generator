...
def test_empty_submission_returns_request_invalid(logged_in_client, generated_paper):
    response = logged_in_client.post(
        "/api/attempts",
        json={"paper_id": generated_paper["paper_id"], "items": []},
    )
    assert response.status_code == 422
    assert response.json()["error_code"] == "request.invalid"


def test_duplicate_item_index_returns_request_invalid(logged_in_client, generated_paper):
    response = logged_in_client.post(
        "/api/attempts",
        json={
    ...
    assert body["detail"]["items"]["duplicate_indices"] == [1]


def test_missing_item_index_returns_request_invalid(logged_in_client, generated_paper):
    response = logged_in_client.post(
        "/api/attempts",
        json={
            "paper_id": generated_paper["paper_id"],
    ...
    assert response.status_code == 422
    assert body["error_code"] == "request.invalid"
    assert body["detail"]["items"]["missing_indices"] == [3]


def test_non_positive_item_index_returns_request_invalid(logged_in_client, generated_paper):
    response = logged_in_client.post(
        "/api/attempts",
        json={"paper_id": generated_paper["paper_id"], "items": [{"index": 0, "user_answer": "A"}]},
    )
    assert response.status_code == 422
    assert response.json()["error_code"] == "request.invalid"
