...
1. **生成页无「出卷模式 / 题量 / 知识点 chips」分段控件**：本仓库产品结构是
   fresh 在生成页、remediation 在错题本页（配合会员门槛与本地错题本）、
   review 在掌握度页（2026-08-07 复盘域拆分），题量/考点由 AI 从自然语言解析。
   属有意保留的产品差异。
2. ~~落地页路由为 `/welcome`~~（2026-08-06 改版，同日二改）：`/` 对所有人都是
   营销首页（已登录时页眉 CTA 变「进入工作台 →」）；工作台在 `/home`，登录后
   直接落到 `/home`（admin 落 `/admin`）；`/welcome` 客户端重定向到 `/`。
   原生成页迁至 `/generate`。访问 `/` 仅发 `GET /auth/me`。
