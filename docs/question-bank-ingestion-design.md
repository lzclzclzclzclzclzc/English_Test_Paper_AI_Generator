...
**题库构建与 AI Engine 的边界**：两者通过共享数据契约（`shared/schemas.py`）和存储层解耦。`shared/schemas.py` **已创建**，是所有跨子系统 pydantic 模型的唯一真相源（single source of truth），ingestion 写、ai_engine 读、backend 序列化都从这里 `import`。题库构建**只写**题库（除幂等去重需要查询外），AI Engine **只读**题库（Solutioner 一处例外，见 Spec B）。
...
- **题型（十种一级类型）**：
  ...
  - `reading_longtext_single_choice`：阅读理解（长文多项单选）
  - `cloze_single_choice`：完形填空
  - `reading_first_blank`：阅读首字母填空
  - `writing`：英语作文（无标准答案，由 WritingGrader 批改）
- **不覆盖**：含图片的题目（原书带图题目在入库时被丢弃）
- **无图片**：所有题目为纯文本

### 1.2 数据源

- **原始形态**：EPUB 电子书 + 补录的听力/阅读/作文素材
- **当前入库**：共 **1428 道题**——原《2021 上海中考试题分类汇编·英语（一模/二模）》
  1066 道，加后续合并的 listening / cloze / reading / writing 等题型。
- **预处理**：脚本转 `.md`（`ebooklib` + `beautifulsoup4` + `html2text`）
- **原始文档不含解析**——解析文本由用户在做题后按需通过 AI Engine 生成（Solutioner 职责，见 Spec B）

### 1.3 存储

- **SQLite（题库）** 存题目主数据、知识点（`data/questions.db`，只读、git 跟踪，bank-only）。
  用户数据（答题记录等）在独立的 `data/app.db`（gitignored），两库不 JOIN——详见 §3.8 与 backend-design.md。
- **ChromaDB**（持久化模式）只存 **3 类自由题型** 的向量（`single_choice` / `word_form` /
  `sentence_rewriting`，见 `shared/schemas.py` 的 `VECTOR_INDEXED_QUESTION_TYPES`）；其余 7 类
  题型（listening_* / cloze / reading_* / writing）为 **SQL-only**，不进向量库。集合 `questions`
  （`data/chroma/`，**已建立**）。
- **Embedding 模型**：本地 Qwen embedding 4B（`Qwen/Qwen3-Embedding-4B`，2560 维，L2 归一化，cosine 距离；模型放 `models/qwen3-embedding-4b/`）

### 1.4 知识点结构

- **两级层次**，扁平表（**无 `parent_id`**：level1 是题型枚举，父子关系已由 level1 值直接表达）
- **一级 = 题型**：即 §1.1 的 10 个题型枚举值
- **二级 = 具体考点**：如"语音"、"介词"、"动词时态与语态"、"改为被动语态"等，共 **56 个**
- **归并方式**：**由开发者手工归并**（原设计的 LLM 归并被评估后放弃：`chapter_l2` 规模小、语义清晰、人工做映射的性价比高于 LLM 归并 + 审核）
...
class KnowledgePoint(BaseModel):
    ...
    level1: QuestionType       # 10 个题型枚举值之一（见 §1.1）
    level2: str                # "动词时态与语态"、"介词" 等
    aliases: list[str] = []    # 同义词（不同书章节标题差异）
```

- 一级 = `QuestionType`（§1.1 的 10 个题型枚举值）；二级由 ingestion 阶段人工归并后固化
...
class Option(BaseModel):
    label: Literal["A", "B", "C", "D", "T", "F"]   # A-D 单选；T/F 听力判断
    text: str
...
class RawQuestion(BaseModel):
    ...
    question_type: QuestionType          # §1.1 的 10 个题型枚举值之一
    ...
    options: list[Option] | None = None  # single_choice / listening_true_false
...
- **单选题**：单字母字符串 `"B"`
- **写作题**：`null`（作文不设标准答案，由 WritingGrader 评分）
...
`embedding_text` 是**入库时不落主库**的派生字符串，只在向量化阶段临时生成并写入 ChromaDB 的 `documents` 字段。实现：`ingestion/chromadb/embedding_text.py::build_embedding_text()`（纯函数，输入 `Question` dict + KP 索引，输出字符串）。
...
```
EPUB → md → RawQuestion JSON → 人工归并 KP 树 → apply_kp → assign_ids → SQLite → ChromaDB
                                        ↑                                     ↓         ↓
                              (审核 knowledge_tree_draft.html          questions.db  chroma/
                              → knowledge_tree.json)                                (可检索)
```
...
| 5 | 入库 ChromaDB | `build-vec` | 需 Qwen 4B | SQLite → embedding_text → 向量 → `data/chroma/` |

每一步产物落盘、可断点重跑、幂等。检索验证命令 `search-vec` 见 §3.10。
...
- 事务包裹：任何一行失败整体回滚
- 本 Loader 只建**题库表**（knowledge_points / questions / question_knowledge_points）；用户数据表（attempts 等）已迁至 data/app.db，见 §3.8
...
```sql
-- 十个题型枚举值（KP 与 questions 的 CHECK 约束共用）
...
CREATE TABLE knowledge_points (
    ...
    level1        TEXT NOT NULL
                    CHECK (level1 IN ('single_choice', 'word_form', 'sentence_rewriting',
                                      ...
                                      'listening_fill_blank', 'reading_longtext_single_choice',
                                      'cloze_single_choice', 'reading_first_blank', 'writing')),
...
CREATE TABLE questions (
    ...
    -- 长文题共享素材（非 passage 题型为 null）
    passage_id          TEXT,                   -- passage group id
    passage_json        TEXT,                   -- JSON Passage 对象

    -- 写作题专属字段（非 writing 题型为 null）
    reference_expressions  TEXT,                -- 参考表达
    min_words             INTEGER,              -- 最低词数要求

    answer_json         TEXT,                   -- writing: null；单选 "\"B\""；填空是复杂结构
...
-- 注意：本 schema 是**题库专用（bank-only）**。用户数据表（attempts /
-- attempt_items / users / sessions / papers / study_plans / vocabulary_*）
-- 自 DB 拆分后**不在此库**，改由 shared/storage.py::init_db() 建在独立的
-- data/app.db 中（gitignored）。
```

**关键调整**（相对原 §3.7）：
- `knowledge_points` 删掉 `parent_id`（KP 树扁平，无父子关系）
- `knowledge_points` / `questions` 的 CHECK 约束覆盖全部 **10 个题型枚举值**
- `questions` 删掉 `difficulty` 和 `embedding_text`（§1.7 / §2.5）
- `questions` 把 `chapter` 拆成 `chapter_l1` + `chapter_l2` + `number`
- `questions` 新增题型专属列：`hint / original_sentence / instruction / template`（原 spec 假设都塞 `stem`，实际发现分开更好查询）
- `questions` 新增长文题列 `passage_id / passage_json` 与写作题列 `reference_expressions / min_words`
- **用户数据表迁出**：`attempts` / `attempt_items` 及后续新增的用户/词表等表**已移到 data/app.db**（DB 拆分），本库为 bank-only

### 3.9 阶段 5：build-vec（`ingestion/chromadb/loader.py`）

**已完成。** 读 SQLite → 每题生成 embedding_text → Qwen 4B 编码 → 写入 ChromaDB。

**三个组件**：

- `ingestion/chromadb/embedding_text.py`：`build_embedding_text()` 纯函数，按 §2.5 模板拼字符串（多 KP 分行、剥 `<u>` 标签、空占位符原样保留）
- `ingestion/chromadb/embedder.py`：`QwenEmbedder` 封装 `sentence-transformers`，从本地 `models/qwen3-embedding-4b/` 加载（无网络下载）、CUDA、batch=16、输出 2560 维 L2 归一化向量
- `ingestion/chromadb/loader.py`：编排 + 幂等 + `search()` 检索辅助函数

**集合 `questions` 结构**（Spec §3.8）：

- `ids`：`Question.id`（如 `"q_00042"`）
- `embeddings`：Qwen 4B 输出，2560 维，L2 归一化
- `documents`：**embedding_text（按 §2.5 模板临时生成，不落主库）**——保留供调试和换模型重跑
- `metadatas`：
  ```python
  {
      "question_type": "single_choice",
      "book": "shanghai_2021_yimo",
      "chapter_l1": "1 单项选择",
      "chapter_l2": "1.1 语音",
...
**距离度量**：集合以 `hnsw:space=cosine` 创建，配合编码时的 L2 归一化，端到端一致。

**幂等**：按 `Question.id` 去重——重跑时集合里已存在的 id 直接跳过，不重复编码。要重建某题向量需先从集合删除该 id（或删 `data/chroma/`）。

**当前规模**：1066 条向量全部写入。

**Retriever 检索约定**（供 Spec B）：
- cosine 距离对同空间的 query 向量
- `where={"question_type": "single_choice"}` 做属性硬过滤
- `kp_ids` 存逗号分隔字符串——Retriever 用 Python 端二次过滤（拿回候选后 `.split(",")` 精确匹配，避免子串误命中）
- **不再按 `difficulty` 过滤**（该字段已删）

**检索验证工具**：CLI `search-vec` 命令（见 §3.10）——不是 AI Engine 的正式 Retriever，只用于建库后 sanity check。
...
# 6. 入库 ChromaDB（加载 models/qwen3-embedding-4b/，CUDA，几分钟）
python -m ingestion.cli build-vec

# 验证：检索测试（不重复 embed，只做一次 query）
python -m ingestion.cli search-vec "现在完成时" -n 5
python -m ingestion.cli search-vec "改为被动语态" -n 5 --qt sentence_rewriting
```

> Windows 下所有命令前加 `PYTHONIOENCODING=utf-8` 避免中文输出乱码。

- 每一步产物落盘、可断点重跑
- 步骤 3b/3c/4/5 幂等
- 步骤 4（build-sqlite）之前允许手工编辑 JSON（用于修补 EPUB 转换损失、划线标注等——见 §3.11）
...
│   ├── build_vocabulary_wordlist.py  # 生成上海词表 JSON 种子
│   ├── build_merged_vocabulary.py    # 合并国家核心 + 上海扩展 → JSON 种子
│   │                                 #   （两脚本产 JSON，经 shared/storage.py 导入
│   │                                 #    data/app.db 的 vocabulary_wordlists /
│   │                                 #    vocabulary_words 表）
│   ├── sqlite/
│   │   ├── __init__.py
...
│   └── chromadb/
│       ├── __init__.py
│       ├── embedding_text.py         # §2.5 模板拼接（纯函数）
│       ├── embedder.py               # Qwen4B 封装（sentence-transformers/cuda）
│       └── loader.py                 # 阶段 5 向量化 + search() 辅助
│
├── models/                           # 本地模型；git 忽略
│   └── qwen3-embedding-4b/           # 预下载的 Qwen3-Embedding-4B
│
├── ai_engine/                        # 见 Spec B
│
├── shared/                           # 跨子系统契约与设施
│   ├── __init__.py
│   ├── schemas.py                    # 全部跨子系统 pydantic 模型（唯一真相源）
│   └── storage.py                    # app.db 存储层（用户数据 / 词表 / 答题记录）
│
├── data/                             # 运行时产物；git 忽略
│   ├── books/*.epub                  # 原始 EPUB
...
- `ingestion/sqlite/` 和 `ingestion/chromadb/` 是新增子目录
- `shared/llm/` **未创建**（未来 Spec B 添加）
- `shared/schemas.py` **已创建**，是所有跨子系统 pydantic 模型的唯一真相源（模型已从 `chapter_splitter.py` 抽出）
- `shared/embedding.py` **未创建**：embedding 逻辑当前在 `ingestion/chromadb/embedder.py`；未来 AI Engine 的 Retriever 需要复用编码器时再抽到 `shared/`
...
8. ✅ 阶段 5：入库 ChromaDB（`chromadb/` 三件套；1066 条向量，Qwen4B 2560 维）

**未完成**：

9. ⬜ `shared/` 抽出（数据契约、配置、存储客户端）——等 AI Engine 启动时做
10. ⬜ 端到端 live-smoke 测试
11. ⬜ ChromaDB loader 的单元测试（用小规模 fixture + stub embedder）

M1 交付：题库 **SQLite + ChromaDB 双库稳态、可检索**；AI Engine（M2/M3/M4）可以开始（见 Spec B，待撰写）。
...
1. **Qwen embedding 4B**：**已解决**——模型放在 `models/qwen3-embedding-4b/`，CUDA 加载，1066 题已全部向量化。检索质量待 Retriever 上线后系统评估
2. **检索质量评估**：`search-vec` 只做了 sanity check；embedding_text 模板（多 KP 分行、剥标签、填空题格式）是否是最优拼法，需要在 AI Engine Retriever 落地后用 golden set 回归验证（见 Spec B）
3. **EPUB 结构差异**：`epub_to_md.py` 里的 `CLASS_TO_HEADING` 目前只覆盖两本上海教辅系列——加新书前需要检查
4. **知识点树版本管理**：当前假设 KP 树入库后不改；若真要改（合并、拆分、改 id），需要专门迁移脚本 + 级联更新 `question_knowledge_points` 表 + **重跑受影响题目的向量**（KP 变了 embedding_text 就变了）
5. **手工补丁的可重放性**：题库最终态包含 100+ 处手工补丁，不能靠"从 md 重跑"完全重生（见 §3.11）——**目前把 chapters/*.json 视为"手工审核后的稳定态"**，任何改动必须同步维护
6. **变体去重**：352 道变体（`number` 含 `-`）当前作为独立题目入库；未来若发现"变体和主题目重复率过高"可能需要人工去重
...
- §3.9 ChromaDB 集合结构 + `chromadb/embedder.py` 编码器（Retriever 复用同一模型编码 query）
...
| `shared/schemas.py` | 存在 | **已创建**，是所有跨子系统 pydantic 模型的唯一真相源 | AI Engine 启动时抽出 |
| assign-ids 阶段 | 无 | **新增** | 需要稳定全库 id |
| viz_knowledge_tree | 无 | **新增** | 辅助人工审核 KP 树 |
| ChromaDB 编码器位置 | `shared/embedding.py` | **`ingestion/chromadb/embedder.py`** | 先随 ingestion；未来 Retriever 需要时抽到 shared |
| ChromaDB 集合距离 | 未指定 | **cosine + L2 归一化** | 与 Qwen 官方推荐一致 |
