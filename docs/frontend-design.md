...
- **21 个用户面页面 + 7 个管理后台页面文件**（2026-08 功能拆分改版后全面重建，早期"八个页面"已过时）：
  - 用户面（21）：**营销首页 LandingPage / 登录 LoginPage / 工作台 DashboardPage /
    ...
    学习计划 StudyPlanPage / 设置 SettingsPage / 会员 MembershipPage /
  ...
  - 管理后台（7 个 tsx 文件，6 条路由）：**AdminLayout（外壳）+ AdminOverviewPage /
    ...
    AdminMembershipsPage / AdminOrdersPage**（详见 Spec G）。
...
frontend/
  ...
  src/
    ...
    api/
      ...
      admin.ts               # 管理后台：概览 / 用户 / 会员 / 订单（走 /api/admin）
...
| `/report` | ReportPage | 是 | 学情报告（会员，打印友好） |
...
| `/admin/memberships` | AdminMembershipsPage | 是（admin） | 会员 |
...
「我的」组不进侧栏分组，收进**底部头像个人菜单**（Sidebar.tsx）：历史试卷 / 会员 / 设置 / 登出。
管理员侧栏走 `ADMIN_GROUPS`（管理后台：概览 / 分析 / 用户 / 会员 / 订单）。
...
- **管理员路由**：`/admin/*` 在 `<RequireAuth>` 之内再套一层 `<RequireAdmin>`（`user.role === 'admin'` 才放行，否则重定向首页），侧栏「管理后台」入口亦按 role 显隐。前端 role 判断仅控制可见性，真正鉴权由后端 `require_admin` 兜底。管理后台共 **6 条路由 / 7 个 tsx 文件**（AdminLayout 外壳 + 概览 / 分析 / 用户 / 用户详情 / 会员 / 订单），页面与接口详见 **Spec G（`admin-design.md`）**。
...
- 44rem `<textarea>` + 建议 chips + 主按钮「生成试卷」，只做 fresh 模式
  （错题巩固 / 综合复习在错题本页 / 掌握度页）；非会员显示当日免费次数。
...
- 复盘态：只读展示作文原文 + 批改结果，由嵌套的 **WritingGradeDisplay** 渲染：
  ...
  详细评析（内容 / 语言 / 结构评析、总体评价、修改范文）为会员专属——非会员在 blur
  遮罩下渲染占位示例并叠加「开通会员查看」层。批改数据由 `writingGradeResult` prop 注入
  （来自 `api/writing.ts`）。
