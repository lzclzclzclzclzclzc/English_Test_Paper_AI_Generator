...
describe('DRILL_CONFIGS 完备性', () => {
  ...
  it('KP/主题仅语法族开放', () => {
    for (const c of DRILL_CONFIGS) {
      expect(c.supportsKp).toBe(c.family === 'grammar')
      expect(c.supportsTopic).toBe(c.family === 'grammar')
    }
  })
  it('阅读首字母锁定 original,其余三档可选', () => {
    for (const c of DRILL_CONFIGS) {
      if (c.type === 'reading_first_blank') {
...
describe('drillBySlug / DRILL_FAMILIES', () => {
  ...
  it('三族分区各 3 个,顺序语法→听力→阅读', () => {
    expect(DRILL_FAMILIES.map((f) => f.family)).toEqual(['grammar', 'listening', 'reading'])
    for (const f of DRILL_FAMILIES) expect(f.configs).toHaveLength(3)
  })
})
