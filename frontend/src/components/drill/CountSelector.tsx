      ...
      <div className="flex flex-wrap items-center gap-x-5 gap-y-3">
        <div className="flex items-center gap-2">
          {presets.map((n) => (
            <button
              ...
              className="seg min-w-11 tabular-nums"
              onClick={() => onChange(n)}
            >
              {n}
            </button>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            aria-label="减少题量"
            disabled={value <= 1}
            className="flex size-8 items-center justify-center rounded-sm border border-hairline font-ui text-[15px] leading-none text-muted-ink transition-colors hover:border-ink hover:text-ink disabled:pointer-events-none disabled:opacity-40"
