...
describe('admin api base-url routing', () => {
  ...
  it('grants membership via /api', async () => {
    const spy = vi.spyOn(globalThis, 'fetch').mockResolvedValue(
      new Response(JSON.stringify({ user_id: 'u', username: null, expires_at: null, active: true }), { status: 200 }),
    )
    await grantMembership('u', { days: 30 })
    expect(spy).toHaveBeenCalledWith(expect.stringContaining('/api/admin/memberships/u/grant'), expect.anything())
  })
})
